...
local is_s4jj4dHk = function(msg)
  local var = false
  if msg.sender_user_id_ == tonumber(sajjad) then
    var = true
  end
  return var
end
...
local is_leaderid = function(user_id)
  ...
  if user_id == tonumber(sajjad) then
    var = true
  end
  return var
end
...
The *Number* of *Warnings* user : `[]] .. warns .. "/" .. sencwarn .. "]`", 1, "md")
      ...
      if is_momod(msg.sender_user_id_, msg.chat_id_) and text:match("^[Ii]d (.*)$") and check_user_channel(msg) then
        local id_by_men = function(extra, result)
          if result.id_ then
            if database:get("lang:gp:" .. msg.chat_id_) then
              if tonumber(result.id_) == tonumber(sajjad) then
                t = "Developer"
...
> <b>ID</b> : <code>]] .. result.id_ .. "</code>"
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and (text:match("^[Aa]ddhelper$") or text:match("^دعوت هلپر$")) then
          ...
          resolve_username("SiSRobot", Invhelper)
        end
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
        ...
        if is_leader(msg) and (text:match("^[Uu]pdate$") or text:match("^بروزرسانی$")) then
          if not database:get("lang:gp:" .. msg.chat_id_) then
            send(msg.chat_id_, msg.id_, 1, "*•نسخه مورد استفاده شما تهیه شده توسط sajjad\\_021 می باشد.\nبرای اطلاع از اپدیت به این کانال مراجعه فرمایید*: @tgMember", 1,"md")
          else
            send(msg.chat_id_, msg.id_, 1, "_• You are using the version provided by 'tgMember'_\n_Check out this Channel for updates :_ @tgMember", 1, "md")
          end
        end
        if is_leader(msg) and (text:match("^[Vv]ersion$") or text:match("^نسخه فعلی$")) then
          local LastVer = "tgMember .Ver"
          if LastVer then
            if not database:get("lang:gp:" .. msg.chat_id_) then
